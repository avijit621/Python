...
def BestfitPolynomial(input_data, degree):
    ...
    y_index = 0
    b = []
    # calculation of RHS of the linear equation
    ...
    y1 = [p[i] for i in x1]
    fig, ax = plt.subplots()
    ax.plot(x1, y1, label="Polynomial approximation")
    ax.plot(x, y, "ro", label="Actual plots")
    ax.set_xlabel("x")
    ax.set_ylabel("y")
    ax.set_title("Least square polynomial fit for a given set of points")
...
BestfitPolynomial([(1, 1), (2, 4), (3, 9), (4, 16)], 3)
